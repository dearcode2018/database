...
public class DataSourceConfiguration {
	...
	@Value("${datasource.default.statement.timeoutSecond:600}")
	...
	public PlatformTransactionManager transactionManager(
		 ...
		 * 同时申请事务(连接)不能超过该时间，否则报超时异常. 申请事务，本质就是从数据源获取连接，然后创建事务对象，检查各类条件，例如是否超时.
		 * 
		 */
	 ...
	 */
	@Bean
	public SqlSessionFactory setSqlSessionFactory(final DataSource dataSource)
			throws Exception {
		VFS.addImplClass(SpringBootVFS.class);
		final SqlSessionFactoryBean bean = new SqlSessionFactoryBean();
		bean.setDataSource(dataSource);
		/*
		 * final org.apache.ibatis.session.Configuration config = new
	...
	public org.apache.ibatis.session.Configuration configuration() {
		...
		configuration.setDefaultStatementTimeout(defaultStatementTimeoutSecond);
	...
	public DruidDataSource buildDruidDataSource(
		...
		dataSource.setMaxEvictableIdleTimeMillis(
				properties.getMaxEvictableIdleTimeMillis());
		dataSource.setValidationQuery(properties.getValidationQuery());
		dataSource.setValidationQueryTimeout(
				properties.getValidationQueryTimeout());
		...
		dataSource.setMaxOpenPreparedStatements(
				properties.getMaxOpenPreparedStatements());
		dataSource.setSharePreparedStatements(
				properties.isSharePreparedStatements());
